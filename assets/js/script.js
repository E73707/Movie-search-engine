...
const errorModal = new bootstrap.Modal(
  document.getElementById("api-error-modal")
);
...
function loadLocalStore() {
  let localStoreBroadSearch250 = JSON.parse(
    localStorage.getItem("broadSearch250")
  );
...
function SearchSubmit() {
  searchBtn.addEventListener("click", () => {
    const rawSearchQuery = keywordSearchQueryBox.value
      .replace(",", " ")
      .replace(".", " ")
      .trim()
      .split(" "); // e.g. "gun,  war " => ['gun', '', '', 'war']
    ...
    for (var i = 0; i < rawSearchQuery.length; i++) {
      if (rawSearchQuery[i] !== " ") {
        // This conditional statement accounts for possible empty strings, i.e., only push non-empty strings
    ...
    if (advancedSearchShow) {
      ...
      const durationList = durationLabelElem.innerText
        .replace(" min", "")
        .split("-");

      const genresSelected = [];
      const genreElemList = document.querySelectorAll(
        'input[id^="genre-btn-"]'
      );
...
function searchMoviePlot(normalSearchQuery) {
  if (
    !broadSearch250.timestamp ||
    dayjs().diff(dayjs(broadSearch250.timestamp), "day") > 1
  ) {
    // Fetch new data if localStorage is empty or the existing data on localStorage is older than 1 day
    console.log("fetch new data from API");
    fetch(
      `https://imdb-api.com/API/AdvancedSearch/k_lyhir636?title_type=feature&user_rating=7.0,10&num_votes=50000,&languages=en&count=250&sort=user_rating,desc`
    )
      ...
      .then(function (data) {
        if (!data) {
          ...
          // i.e., if fetched data is an empty array
          apiErrorMsg.innerText =
            "Your search returns no result. Please try again";
          errorModal.show();
          return;
        } else {
  ...
  for (let i = 0; i < normalSearchQuery.length; i++) {
    for (let j = 0; j < broadSearch250.result.length; j++) {
      ...
      if (moviePlot.search(normalSearchQuery[i]) !== -1) {
                                        ...
                                        <div class="col-6 col-lg-4 col-xl-3">
                                            <div class="card shadow-lg my-3 bg-dark">
                                                ...
                                                <div class="card-body">
                                                    ...
                                                    <a onclick="playTrailer('${movieId}', '${plot.replace(
      "'",
      "\\'"
    )}','${movieTitle.replace(
      "'",
      "\\'"
    )}')" href="#click-trailer" class="btn trailer-btn mt-2">Trailer</a>
                                                    <br>
...
function playTrailer(movieId, plot, movieTitle) {
  ...
  const enableMovieTrailerModal = new bootstrap.Modal(
  ...
  fetch(`https://imdb-api.com/en/API/YouTubeTrailer/k_lyhir636/${movieId}`)
    ...
    .then(function (data) {
      if (!data) {
        ...
        let trailerHTML = `<iframe width="100%" height="500px" src="https://youtube.com/embed/${
          data.videoUrl.split("?v=")[1]
        }" alt="Movie Trailer"></iframe>
...
function advancedSearchSubmit(advancedParam) {
  ...
  fetch(
    `https://imdb-api.com/API/AdvancedSearch/k_lyhir636?title_type=feature&user_rating=${minUserRating},${maxUserRating}&release_date=${minYear},${maxYear}&num_votes=${minUserVotes},${maxUserVotes}&genres=${genreArray}&languages=en&count=250&sort=user_rating,desc`
  )
    .then(function (response) {
      if (response.status === 404) {
        throw new Error("API Not found");
      ...
      if (!data) {
        ...
        if (runtime.length !== 0) {
          ...
          for (var i = 0; i < data.results.length; i++) {
            if (data.results[i].runtimeStr !== null) {
              // In some data objects, runtimeStr and/or plot value is null. This app is designed to exclude these objects from the advancedFilterList array.
              let runtimeData = Number(
                data.results[i].runtimeStr.split(" ")[0]
              );
              if (
                runtimeData >= Number(runtime[0]) &&
                runtimeData <= Number(runtime[1])
              ) {
                if (plotSearch.length !== 0) {
                  for (var j = 0; j < plotSearch.length; j++) {
                    if (
                      data.results[i].plot !== null &&
                      data.results[i].plot.search(plotSearch[j]) !== -1
                    ) {
                      const foundIndex = advancedFilterList.findIndex(
                        (item) => {
                          // This is to prevent duplicates in advancedFilterList array.
...
function renderGenreSelectionBtn() {
  let genres =
    "Action,Comedy,Family,Sci-Fi,War,Adventure,Fantasy,Horror,Animation,Drama,Romance,Thriller";
  ...
  for (var i = 0; i < genresArr.length; i++) {
    ...
    let genreBtn = `<div class="mb-2 me-2">
                            <input id="genre-btn-${i + 1}" data-genre="${
      genresArr[i]
    }" type="checkbox">
                            <label class="genre-checkbox" for="genre-btn-${
                              i + 1
                            }" class="text-white">${genresArr[i]}</label>
                        </div>`;
